...
import { StockWizard } from '@/components/stock/stock-wizard';
...
function StockContent() {
  ...
  const [isCurrentDayClosed, setIsCurrentDayClosed] = useState(false);
  ...
  useEffect(() => {
    ...
    apiService.getDailyState(date)
      .then((state) => {
        ...
        const closed = !!(state.keg && (state.keg.closing !== null || state.keg.total_money !== null));
        setIsCurrentDayClosed(closed);
        setChecking(false);
      })
      .catch(() => {
        setIsPreviousDayResolved(true);
        setIsCurrentDayClosed(false);
      ...
      {!isPreviousDayResolved ? (
        <div className="card border border-danger/30 bg-white flex flex-col gap-4 p-6 shadow-md">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-primary/70 mt-1">
                You cannot enter stock for <strong className="text-primary">{date}</strong> because the previous day <strong className="text-danger">{previousDate}</strong> has not been closed/balanced.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                {skipping ? 'Skipping...' : 'Skip Day & Proceed'}
              </button>
            </div>
          </div>
          <Link
            href={`/?date=${previousDate}`}
            className="btn-primary text-center py-3 px-6 mt-2"
          >
            Go to {previousDate} to balance it
          </Link>
        </div>
      ) : isHistorical ? (
        <div className="card border border-danger/30 bg-white flex flex-col gap-4 p-6 shadow-md text-center py-12">
          ...
          <p className="text-primary/70">
            You are viewing a past date (<strong>{date}</strong>). You cannot add stock or modify sales for a day that precedes your latest active day, as this would break the stock carry-forward cycle.
          </p>
          <div className="mt-4">
            <Link
              href={`/`}
        ...
        <div className="card border border-danger/30 bg-white flex flex-col gap-4 p-6 shadow-md text-center py-12">
          ...
          <h3 className="font-black text-2xl text-primary">Day Already Closed</h3>
          <p className="text-primary/70">
            You have already closed sales for <strong>{date}</strong>. You cannot modify or add stock for a day that is already closed.
          </p>
          <div className="mt-4">
            <Link
              href={`/`}
        ...
        <Suspense fallback={<div className="h-40 animate-pulse bg-primary/5 rounded-2xl" />}>
          <StockWizard date={date} />
        </Suspense>
      )}
    </div>
  );
