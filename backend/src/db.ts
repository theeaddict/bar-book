...
const tenantInitLocks = new Map<string, Promise<void>>();
 ...
 */
async function ensureTablesForTenant(tenantId: string): Promise<void> {
 ...
 */
async function ensureTablesExist(client: PoolClient, tenantId: string) {
  // settings
  await client.query(`
...
export async function auditLog(
  ...
  await client.query(
    `INSERT INTO audit_logs (tenant_id, username, action, table_name, record_id, payload)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [tenantId, username, action, tableName, recordId, JSON.stringify(payload)]
  );
}
