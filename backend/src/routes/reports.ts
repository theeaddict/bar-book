...
async function getDailySummaryForDate(client: any, tenantId: string, date: string) {
  // Fetch day's products
  const { rows: dayProducts } = await client.query(
    `SELECT dp.opening::float AS opening, dp.added::float AS added, dp.left_count::float AS left_count, dp.buy_price, dp.sell_price
  ...
  // Fetch day's keg
  const { rows: dayKegs } = await client.query(
    `SELECT opening::float AS opening, added::float AS added, closing::float AS closing, buy_price, sell_price, total_money, overflow::float AS overflow, expenses::float AS expenses
     FROM day_kegs
...
async function getSummariesForDateRange(client: any, tenantId: string, dates: string[]) {
  if (dates.length === 0) return [];

  const { rows: dayProductsRows } = await client.query(
    `SELECT dp.date::text AS date, dp.opening::float AS opening, dp.added::float AS added,
            dp.left_count::float AS left_count, dp.buy_price, dp.sell_price
     FROM day_products dp
     WHERE dp.tenant_id = $1 AND dp.date = ANY($2::date[])`,
    [tenantId, dates]
  );

  const { rows: dayKegsRows } = await client.query(
    `SELECT dk.date::text AS date, dk.opening::float AS opening, dk.added::float AS added,
     ...
     FROM day_kegs dk
     WHERE dk.tenant_id = $1 AND dk.date = ANY($2::date[])`,
    [tenantId, dates]
  );

  // Index by date
  const prodsByDate: Record<string, any[]> = {};
  for (const r of dayProductsRows) {
    if (!prodsByDate[r.date]) prodsByDate[r.date] = [];
    prodsByDate[r.date].push(r);
  }
  const kegsByDate: Record<string, any> = {};
...
router.get('/week', async (req, res, next) => {
  try {
    ...
    const reports = await withTenant(tenantId, async (client) => {
...
router.get('/transactions', async (req, res, next) => {
  try {
    ...
    const transactions = await withTenant(tenantId, async (client) => {
      const { rows: bottledRows } = await client.query(
      ...
      const { rows: kegRows } = await client.query(
        `SELECT opening::float AS opening, added::float AS added, closing::float AS closing, buy_price, sell_price
         FROM day_kegs
         WHERE tenant_id = $1 AND date = $2`,
        [tenantId, date]
      );
  ...
  try {
    const rows = await withTenant(tenantId, async (client) => {
      const { rows } = await client.query(
        `SELECT id, username, action, table_name, record_id, payload, created_at 
         FROM audit_logs 
